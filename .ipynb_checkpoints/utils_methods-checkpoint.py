...
### Methods
def train_FedAvg(data_obj, act_prob ,learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, hpo_method, hpo_epoch, hpo_maxiter, hpo_per, rand_seed=0):
   
    method_name = 'FedAvg_%s' % hpo_method
    ...
    # HPO Structures
    clnt_learning_rates = [learning_rate] * n_clnt
    hyperparameters = np.zeros((com_amount, 1))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        hyperparameters = np.load('Output/%s/%s/%d_hyperparameters.npy' %(data_obj.name, method_name, com_amount))
        ...
        for i in range(com_amount):
            ...
            # Determine if HPO is performed in this communication round         
            if i % hpo_per == 0 and hpo_method=='nm':
                with_hpo = True
                print("Hyperparameter Optimization will be performed this round!")
            else:
                with_hpo = False
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt], clnt_learning_rates[clnt] = train_model(clnt_models[clnt], trn_x, trn_y, learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, hpo_method, hpo_epoch, hpo_maxiter, with_hpo, data_obj.dataset)

                clnt_params_list[clnt] = get_mdl_params([clnt_models[clnt]], n_par)[0]
                
                print("Client learning rate:", clnt_learning_rates[clnt])
                
            # Update hyperparameters
            
            if hpo_method=='nm':
                sub_lr = np.take(clnt_learning_rates, selected_clnts)
                learning_rate = np.mean(sub_lr)
                print(learning_rate)
                hyperparameters[i] = [learning_rate]
                print("NM!")
            elif hpo_method=='decay':
                hyperparameters[i] = learning_rate * (lr_decay_per_round ** i)
            elif hpo_method=='hpona':
                hyperparameters[i] = learning_rate
            ...
            if ((i+1) % save_period == 0):
                ...
                np.save('Output/%s/%s/%d_hyperparameters.npy' %(data_obj.name, method_name, (i+1)), hyperparameters[:i+1])
                ...
                if (i+1) > save_period:
                    if os.path.exists('Output/%s/%s/%d_com_trn_perf_sel.npy' %(data_obj.name, method_name, i+1-save_period)):
                        ...
                        os.remove('Output/%s/%s/%d_hyperparameters.npy' %(data_obj.name, method_name, i+1-save_period))
    ...
    return fed_mdls_sel, trn_perf_sel, tst_perf_sel, fed_mdls_all, trn_perf_all, tst_perf_all, hyperparameters


def train_SCAFFOLD(data_obj, act_prob, learning_rate, batch_size, n_minibatch, com_amount, print_per, weight_decay, model_func, init_model, save_period, lr_decay_per_round, hpo_method, hpo_n_minibatch, hpo_maxiter, hpo_per, rand_seed=0, global_learning_rate=1):
    
    method_name = 'Scaffold_%s' % hpo_method
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt], clnt_learning_rates[clnt] = train_scaffold_mdl(clnt_models[clnt], model_func, state_params_diff_curr, trn_x, trn_y, learning_rate * (lr_decay_per_round ** i), batch_size, n_minibatch, print_per, weight_decay, hpo_method, hpo_n_minibatch, hpo_maxiter, with_hpo, data_obj.dataset)
...
def train_FedDyn(data_obj, act_prob, learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, alpha_coef, save_period, lr_decay_per_round, hpo_method, hpo_epoch, hpo_maxiter, hpo_per, rand_seed=0):
    
    method_name  = 'FedDyn_%s' % hpo_method
    ...
    # HPO Structures
    clnt_learning_rates = [learning_rate] * n_clnt
    clnt_alpha_coefs = [alpha_coef] * n_clnt
    hyperparameters = np.zeros((com_amount, 2))
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_models[clnt], clnt_learning_rates[clnt], clnt_alpha_coefs[clnt] = train_feddyn_mdl(model, model_func, alpha_coef_adpt, cld_mdl_param_tensor, local_param_list_curr, trn_x, trn_y, learning_rate * (lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, hpo_method, hpo_epoch, hpo_maxiter, with_hpo, data_obj.dataset)
                ...
                # print("Client learning rate:", clnt_learning_rates[clnt])
                # print("Client alpha:", clnt_alpha_coefs[clnt])
                
            # Update hyperparameters
            
            if hpo_method=='nm':
                sub_lr = np.take(clnt_learning_rates, selected_clnts)
                sub_a = np.take(clnt_alpha_coefs, selected_clnts)
                learning_rate = np.mean(sub_lr)
                alpha_coef = np.mean(sub_a)
                hyperparameters[i] = [learning_rate, alpha_coef]
            elif hpo_method=='decay':
                hyperparameters[i] = [learning_rate * (lr_decay_per_round ** i), alpha_coef]
            elif hpo_method=='hpona':
                hyperparameters[i] = [learning_rate, alpha_coef]
            
            # print("Global learning rate:", learning_rate)
            # print("Global alpha:", alpha_coef)
    ...
    return fed_mdls_sel, trn_perf_sel, tst_perf_sel, fed_mdls_all, trn_perf_all, tst_perf_all, fed_mdls_cld, hyperparameters

def train_FedProx(data_obj, act_prob ,learning_rate, batch_size, epoch, com_amount, print_per, weight_decay, model_func, init_model, save_period, mu, lr_decay_per_round, hpo_method, hpo_epoch, hpo_maxiter, hpo_per, rand_seed=0):
    
    method_name = 'FedProx_%s' % hpo_method
    ...
    if os.path.exists('Output/%s/%s/%d_com_tst_perf_all.npy' %(data_obj.name, method_name, com_amount)):
        ...
        for i in range(com_amount):
            ...
            for clnt in selected_clnts:
                ...
                for params in clnt_models[clnt].parameters():
                    params.requires_grad = True
                clnt_models[clnt], clnt_learning_rates[clnt], clnt_mus[clnt] = train_fedprox_mdl(clnt_models[clnt], avg_model_param_tensor, mu, trn_x, trn_y, learning_rate*(lr_decay_per_round ** i), batch_size, epoch, print_per, weight_decay, hpo_method, hpo_epoch, hpo_maxiter, with_hpo, data_obj.dataset)
                clnt_params_list[clnt] = get_mdl_params([clnt_models[clnt]], n_par)[0]
                
                # print("Client learning rate:", clnt_learning_rates[clnt])
                # print("Client mu:", clnt_mus[clnt])

            # Update Hyperparameters
            
            if hpo_method=='nm':
                sub_lr = np.take(clnt_learning_rates, selected_clnts)
                sub_m = np.take(clnt_mus, selected_clnts)
                learning_rate = np.mean(sub_lr)
                mu = np.mean(sub_m)
                hyperparamters[i] = [learning_rate, mu]
            elif hpo_method=='decay':
                hyperparameters[i] = [learning_rate * (lr_decay_per_round ** i), mu]
            elif hpo_method=='hpona':
                hyperparameters[i] = [learning_rate, mu]
